...
public class Transferencia {
    ...
    public Operaciones transferencia(TransferDto transferDto) throws CuentaEstaDeBajaException, CuentaSinDineroException, CuentaNoEncontradaException, CuentaDistintaMonedaException, TransferenciaFailException {
        ...
        if (clienteOrigen.getBanco().equalsIgnoreCase(clienteDestino.getBanco())) {
            transferenciaRealizada = realizarTransferencia(cuentaOrigen, cuentaDestino, datosTransfer, montoConCargo);
        } else {
            transferenciaRealizada = transferenciaBancoExterno(cuentaOrigen, cuentaDestino, datosTransfer, montoConCargo);
        }
    ...
    private void validateTransferencia(Cuenta cuentaOrigen, Cuenta cuentaDestino, Transfer datosTransfer, double montoConCargo) throws CuentaEstaDeBajaException, CuentaDistintaMonedaException, CuentaNoEncontradaException, CuentaSinDineroException {
        ...
        if (montoConCargo > cuentaOrigen.getSaldo()) throw new CuentaSinDineroException("No hay suficiente dinero en la cuenta " + cuentaOrigen.getNombre() + ", su saldo es de $" + cuentaOrigen.getSaldo());
    }

    //Funcion para calcular el monto con cargo de la transferencia
    ...
    private Operaciones realizarTransferencia(Cuenta cuentaOrigen, Cuenta cuentaDestino, Transfer datosTransfer, double montoConCargo){
        TipoTransaccion tipoTransaccion = datosTransfer.getTipoTransaccion();
        ...
        cuentaOrigen.setSaldo(cuentaOrigen.getSaldo() - montoConCargo);
        cuentaDestino.setSaldo(cuentaDestino.getSaldo() + datosTransfer.getMonto());

        //Tomo registro de la transferencia en la cuentaOrginen y destino
        movimientosDao.saveMovimiento(tipoOperacion + cuentaDestino.getNombre() + " - " + tipoTransaccion, montoConCargo, cuentaOrigen.getCVU());
        movimientosDao.saveMovimiento(tipoOperacionDestino + cuentaOrigen.getNombre() + " - " + tipoTransaccion, datosTransfer.getMonto(), cuentaDestino.getCVU());

        //Guardo la cuentaOrigen y cuentaDestino modificada
        cuentaDao.saveCuenta(cuentaOrigen);
        cuentaDao.saveCuenta(cuentaDestino);

        return new Operaciones().setCvu(cuentaOrigen.getCVU()).setSaldoActual(cuentaOrigen.getSaldo()).setMonto(montoConCargo).setTipoOperacion(tipoOperacion + cuentaDestino.getNombre() + " - " + tipoTransaccion);
    }

    //Funcion para simular una transferencia de un banco externo
    private Operaciones transferenciaBancoExterno(Cuenta cuentaOrigen, Cuenta cuentaDestino, Transfer datosTransfer, double montoConCargo) throws TransferenciaFailException {
